...
plt_single_temp <- function(x, add = T){
    ...
    terra_plt_ortho(
      ...
      , add_crowns = add
      , title = paste0(
        unt_temp %>% dplyr::slice(x) %>% dplyr::pull(trtmnt_type)
        , "| block "
        , unt_temp %>% dplyr::slice(x) %>% dplyr::pull(trtmnt_block)
        , " | "
        , unt_temp %>% dplyr::slice(x) %>% dplyr::pull(trtmnt_unit)
      )
    )
...
# map over blks
unique(unt_temp$trtmnt_block) %>% 
  .[2] %>% 
  purrr::walk(\(x) plt_blk_temp(x, save = F))
```
