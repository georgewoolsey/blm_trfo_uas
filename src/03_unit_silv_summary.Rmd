...
unq_units_df <- treetops_sf %>% 
  ...
  dplyr::mutate(xxx=trtmnt_type) %>% 
  recode_treatment_type_fn(ordered = T) %>% 
  dplyr::arrange(trtmnt_block,trtmnt_type,trtmnt_unit) %>% 
  dplyr::select(-trtmnt_type) %>% 
  dplyr::rename(trtmnt_type=xxx)
# unq_units_df
...
terra_plt_ortho <- function(
  ...
  , title
  , add_crowns = T
  , col = harrypotter::hp(n=2, option="hermionegranger")[2]
  , linewidth = 0.6
) {
  if(terra::nlyr(ortho)==3){
    ...
    terra::plot(
      ...
      # , mar = c(0.2,0.2,2.5,7) # for printing
      , axes = F
      , plg = list(
        title = "CHM (m)"
        , cex = 0.7, title.cex = 0.7, shrink = 0.6
        # , cex = 1.7, title.cex = 1.7, shrink = 0.68,nudge = c(13, 0) # for printing
      )
      , main = title, cex.main=0.7
    ) 
  }
...
plt_single_temp <- function(x, rgb = T, add = T, add_tit = T, my_col = harrypotter::hp(n=2, option="hermionegranger")[2], my_lwd = 0.6){
...
grDevices::jpeg("../data/plt_rgb_ex.jpeg", width = 10.5, height = 7, units = "in", res = 300)
unq_units_df %>%
  dplyr::mutate(rn=dplyr::row_number()) %>%
  dplyr::filter(trtmnt_block==1,tolower(trtmnt_type)=="uniform- high",tolower(trtmnt_unit)=="u10") %>%
  dplyr::pull(rn) %>%
  plt_single_temp(add = T, add_tit = F, my_col = "cyan", my_lwd = 0.7)
grDevices::dev.off()

# grDevices::jpeg("../data/plt_chm_ex.jpeg", width = 10.8, height = 7.2, units = "in", res = 300)  # for printing
grDevices::jpeg("../data/plt_chm_ex.jpeg", width = 10.5, height = 7, units = "in", res = 300)
unq_units_df %>% 
  dplyr::mutate(rn=dplyr::row_number()) %>% 
  dplyr::filter(trtmnt_block==1,tolower(trtmnt_type)=="uniform- high",tolower(trtmnt_unit)=="u10") %>% 
  dplyr::pull(rn) %>% 
  plt_single_temp(add = T, rgb = F, add_tit = F, my_col = "cyan", my_lwd = 0.7)
grDevices::dev.off()
```
...
let's check out some examples of the RGB by treatment block with the segmented crowns outlined in blue
...
plt_blk_temp <- function(blk, add = F, save = F) {
  ...
  unq_units_df %>% 
    ...
    dplyr::pull(rownum) %>% 
    purrr::walk(\(x) plt_single_temp(x,add=add, my_col = "cyan"))
