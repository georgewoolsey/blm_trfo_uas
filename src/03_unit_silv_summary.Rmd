...
terra_plt_ortho <- function(
  ...
  if(terra::nlyr(ortho)==3){
    terra::plotRGB(ortho, stretch = "lin", mar = c(0.2,0.2,2,0.2)) 
  }else{
    terra::plot(ortho, col = viridis::plasma(n=100), mar = c(0.2,0.2,2,3.5), axes = F) 
  }
  terra::plot(
    boundary %>% 
  ...
  terra::plot(
    crowns %>% 
      sf::st_transform(terra::crs(ortho)) %>% 
      terra::vect()
    , add = T, border = "cyan", col = NA, lwd = 0.5
  )
}
```
