...
## Tree Distribution by Unit

distribution of individual tree metrics by treatment

tree height by treatment type (this groups all trees under the same treatment type together)
...
treetops_sf %>% 
  ...
  ggplot2::geom_density(
    mapping = ggplot2::aes(x = tree_height_m,y=ggplot2::after_stat(scaled))
    , alpha = 0.8, color = NA
  ) +
  ggplot2::geom_vline(
    data = treetops_sf %>% 
      ...
      dplyr::summarise(median_tree_height_m=median(tree_height_m,na.rm=T))
    , mapping = ggplot2::aes(xintercept = median_tree_height_m)
...
tree DBH by treatment type (this groups all trees under the same treatment type together)
...
treetops_sf %>% 
  ...
  ggplot2::geom_density(
    mapping = ggplot2::aes(x = dbh_cm,y=ggplot2::after_stat(scaled))
    , alpha = 0.8, color = NA
  ) +
  ggplot2::geom_vline(
    data = treetops_sf %>% 
      sf::st_drop_geometry() %>% 
      dplyr::group_by(trtmnt_type) %>% 
      dplyr::summarise(median_dbh_cm=median(dbh_cm,na.rm=T))
    , mapping = ggplot2::aes(xintercept = median_dbh_cm)
    , linetype = "dashed", lwd = 1, color = "gray33"
  ) +
  ggplot2::facet_grid(
    rows = dplyr::vars(trtmnt_type)
...
```{r}
list.files("c:/Users/georg/Downloads/", pattern = ".*\\.(R|r)$")
```
