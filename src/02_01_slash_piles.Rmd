...
stand_boundary <- 
  ...
  dplyr::mutate(
    ...
    , type = tretmnt %>% tolower() %>% stringr::str_replace_all("heterogenous","heterogeneous")
...
## Candidate Pile Review{#pile_review}
...
chm_rast %>% 
  terra::crop(
    stand_boundary %>%
      dplyr::filter(tolower(type)!="control") %>% 
      dplyr::slice(1) %>% 
      sf::st_transform(terra::crs(chm_rast)) %>% 
      sf::st_buffer(5) %>% 
      terra::vect()
...
terra::plot(
    stand_boundary %>%
      dplyr::filter(tolower(type)!="control") %>% dplyr::slice(1) %>% terra::vect()
    , add = T, border = "black", col = NA, lwd = 1.5
  )
piles_workflow_ans %>% 
...
### make a function to get clean factor labels for the treatment type
recode_treatment_type_fn <- function(df, short_labels = FALSE, ordered = FALSE) {
  ###################### Heterogeneous
  raw_levels <- c("control", "heterogeneous", "uniform- low", "uniform- high")
  labels_full <- c("Control", "Heterogeneous", "Uniform-Low", "Uniform-High")
  labels_short <- c("C", "H", "UL", "UH")
  
  target_labels <- if (short_labels) labels_short else labels_full

  df %>% 
    dplyr::mutate(
      trtmnt_type = trtmnt_type %>% 
        stringr::str_to_lower() %>% 
        stringr::str_trim() %>% 
        factor(levels = raw_levels, ordered = ordered) %>% 
        forcats::fct_relabel(~ target_labels)
    )
}
```
...
```{r,eval=F,purl=FALSE, fig.height = 11.5}
  ...
  purrr::map(function(x){
    unit_temp <- stand_boundary %>% 
      dplyr::filter(
        ...
        , stringr::str_squish(tolower(type)) == unt_temp$trtmnt_type[x]
      ) %>% 
      sf::st_transform(terra::crs(chm_rast))
...
```{r,eval=F,purl=FALSE, fig.height = 9.8}
# plts_temp[1]
...
```{r,eval=F,purl=FALSE, fig.height = 9}
wplts_temp <- patchwork::wrap_plots(
  plts_temp[13:24]
  , ncol = 4
  , guides = "collect"
...
```{r,eval=F,purl=FALSE, fig.height = 10.7}
wplts_temp <- patchwork::wrap_plots(
  plts_temp[25:36]
  , ncol = 3
  , guides = "collect"
...
```{r}
# scales::show_col(scales::hue_pal()(4)) # not color-blind friendly 
# scales::show_col(RColorBrewer::brewer.pal(n=4,name="Dark2")) # yes color-blind friendly 

pile_sum_temp %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(
    dplyr::across(c(trtmnt_block,trtmnt_unit),as.ordered)
