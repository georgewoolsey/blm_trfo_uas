...
we'll exclude these as well and implement the exclusions using a spatial join with the treatment unit boundaries. we'll also set a minimimum tree height threshold to define what is considered a "tree"
...
we used a minimum tree height of `r min_tree_ht_m` m for validation which we'll also apply to our UAS-detected trees
...
## Detection Accuracy {#detect_metrics_form}
...
reference_prediction_match_ans_temp <- reference_prediction_match(
  ...
  , ref_data = 
    validation_trees %>% 
      dplyr::inner_join(
        validation_plots %>% dplyr::slice(11) %>% sf::st_drop_geometry() %>% dplyr::select(plot)
        , by = "plot"
      ) %>% 
      dplyr::rename(tree_height_m=height_m, dbh_cm=dbh_cm)
