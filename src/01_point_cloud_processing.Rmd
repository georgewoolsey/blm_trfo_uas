...
ctg_temp <- sf::st_read("../data/point_cloud_processing_delivery/raw_las_ctg_info.gpkg")
ctg_temp %>% sf::st_union() %>% sf::st_area() %>% as.numeric() %>% `/`(10000) %>% scales::comma(accuracy = 0.01, suffix = " ha")
...
```{r,eval=F,purl=FALSE, fig.height=6, fig.width=7}
...
```{r,eval=F,purl=FALSE, fig.height=7, fig.width=8.3}
...
plt_ws_fn +
  ggplot2::geom_function(
    fun = my_custom_ws_fn
    , mapping = ggplot2::aes(color = "my_custom_ws_fn")
    , lwd = 2
  )
```
...
itd_tuning_ans2 <- cloud2trees::itd_tuning(
  ...
  , ws_fn_list = list(
    ...
    , my_custom_ws_fn = my_custom_ws_fn
  )
)
```
...
```{r,eval=F,purl=FALSE, fig.height=5.5, fig.width=7.2}
...
### building the plt_rgb_rast_itd_crowns fn
  ...
  terra::plot(
    itd_tuning_ans2$crowns %>%
      cloud2trees::simplify_multipolygon_crowns() %>%
      dplyr::ungroup() %>%
      dplyr::filter(sample_number == 1 & ws_fn == "my_custom_ws_fn") %>%
...
```{r, warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
remove(list = ls()[grep("_temp",ls())])
remove(my_custom_ws_fn, my_ws_fn, plt_rgb_rast_itd_crowns, raw_las_ctg_info, processing_data, plt_ws_fn, pj_rgb_rast)
gc()
```
