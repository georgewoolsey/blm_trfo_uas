...
stand_boundary <- 
  ...
  dplyr::mutate(
    unit_ha = as.numeric(sf::st_area(geometry))/10000
  )
...
pile_sum_temp <- piles_workflow_ans %>% 
  ...
  dplyr::group_by(
    trtmnt_type,trtmnt_block,trtmnt_unit, trtmnt_unit_ha
  ) %>% 
  dplyr::summarize(
    n_piles = dplyr::n()
    , dplyr::across(
  ...
  dplyr::ungroup() %>% 
  dplyr::mutate(piles_per_ha = n_piles/trtmnt_unit_ha) %>% 
  dplyr::arrange(trtmnt_type,trtmnt_block,trtmnt_unit )
```
...
pile_sum_temp %>% 
  dplyr::mutate(
    ...
    , dplyr::across(
      .cols = c(trtmnt_unit_ha, piles_per_ha, tidyselect::ends_with("_mean"))
...
pile_sum_temp %>% 
  ...
  dplyr::summarise(
    dplyr::across(
      .cols = c(trtmnt_unit_ha, n_piles, tidyselect::ends_with("_sum"))
      , ~sum(.,na.rm = T)
    )
  ) %>% 
  dplyr::mutate(
    ...
    , piles_per_ha = n_piles/trtmnt_unit_ha
  ) %>% 
  dplyr::mutate(
    dplyr::across(
      .cols = c(n_piles, tidyselect::ends_with("_sum"))
  ...
  dplyr::select(
   trtmnt_type,trtmnt_unit_ha,n_piles
   , piles_per_ha
